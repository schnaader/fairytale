...
public:
	explicit BitmapParser(void) { priority = PARSER_PRIORITY_BITMAP; }
	bool parse(Block* block, ParseData* data, StorageManager* manager) {
		...
		while (i < l) {
			...
			while (k < bytesRead && i < l) {
				...
				if (
				(
				(wnd[2] & 0xFFFF) == 0x424D /*BM*/ && ((imgOffset = (wnd[0] >> 32)) & 0xB7FBFFFF) == 0x36000000
				&& betoh32(imgOffset) <= (BITMAP_FILE_HEADER + BITMAP_INFO_HEADER + 4 * 256) && (wnd[0] & 0xFFFFFFFF) == 0x28000000)
				|| (!HdrRequired && (noFileHdr = ((wnd[0] & 0xFFFFFFFF) == 0x28000000)))) {
					...
					|| block->data->blockRead(&bmp.colorPlanes, 2) != 2 || letoh16(bmp.colorPlanes) != 1 || block->data->blockRead(&bmp.bpp, 2) != 2
					|| (letoh16(bmp.bpp) != 1 && bmp.bpp != 4 && bmp.bpp != 8 && bmp.bpp != 24 && bmp.bpp != 32)
					|| (data->image.stride = ((data->image.width * (data->image.bpp = (uint8_t)bmp.bpp) + 31) & ~31) >> 3) * data->image.height < 0x100
					|| // skip if less than 256 bytes
					...
					|| block->data->blockRead(&bmp.paletteEntries, 4) != 4
            ...
            (data->image.width==8)   || (data->image.width==10) || (data->image.width==14) || (data->image.width==16) || (data->image.width==20) ||
              ...
              (data->image.width==128) || (data->image.width==256)
					...
					if (
					noFileHdr && bmp.size > 0
					&& int64_t(bmp.size) < (int64_t(data->image.width * data->image.height * bmp.bpp) >> 3)) { /*Guard against erroneous DIB detections*/
					} else {
						...
						if (i + (offset - position) + length <= l) {
							...
							LOG(
							"%dbpp %s%s found at %" PRIu64 ", %dx%d, %" PRIu64 " bytes\n", bmp.bpp, (data->image.grayscale ? "grayscale " : ""),
							(noFileHdr ? "DIB" : "BMP"), offset, data->image.width, data->image.height, length);
