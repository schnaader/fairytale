        ...
        if (block->data->blockRead(&header, sizeof(DDS_HEADER))==sizeof(DDS_HEADER) &&
          ...
          if (header.ddspf.dwFlags==DDPF_FOURCC) {
            ...
            if ((header.ddspf.dwFourCC&0xFF)==0x44)
              LOG("DDS DXT%d texture found at %" PRIu64 ", %ux%u, %u MipMaps, %" PRIu64 " bytes\n", (header.ddspf.dwFourCC>>24)&0xF, position-8, header.dwWidth, header.dwHeight, header.dwMipMapCount, length);
            else
              LOG("DDS ATI%d texture found at %" PRIu64 ", %ux%u, %u MipMaps, %" PRIu64 " bytes\n", (header.ddspf.dwFourCC>>24)&0xF, position-8, header.dwWidth, header.dwHeight, header.dwMipMapCount, length);
          }
          else {
            uint32_t j = 0;
            do {
