...
ModParser::ModParser(void) { priority = PARSER_PRIORITY_MOD; }
...
bool ModParser::parse(Block* block, ParseData* data, StorageManager* manager) {
	...
	while (i < l) {
		...
		while (k < bytesRead && i < l) {
			...
			if (
			index >= 1084 && (last4[1] & 0xC0C0C0C0) == 0 && (last4[0] == 0x4D2E4B2E /*M.K.*/ || last4[0] == 0x4D214B21 /*M!K! (ProTracker, when more than 64 patterns are used*/ || ((last4[0] & 0xF1FFFFFF) == 0x3043484E /*xCHN, x is even*/ && ((last4[0] >> 24) & 0xE) < 10) || last4[0] == 0x464C5434 /*FLT4*/ || last4[0] == 0x464C5438 /*FLT8*/ || (last4[0] & 0xFFF7FFFF) == 0x4F435441 /*OCTA or OKTA*/ || last4[0] == 0x43443831 /*CD81*/ || (last4[0] & 0xFFFFFFFC) == 0x54445A30 /*TDZx, x in 1-3*/ || ((last4[0] & 0xF0F0FFF9) == 0x30304348 && (c == 0x48 || c == 0x4E) && ((last4[0] >> 24) < 0x3A && ((last4[0] >> 16) & 0xFF) < 0x3A) /*xxCH or xxCN, x in 0-9*/))
			&& window[(index - 134) & WINDOW_ACCESS_MASK] <= 0x80) {
				...
				? 8
				: ((last4[0] & 0xFFF9) == 0x4348 /*..CH or ..CN*/) ? ((last4[0] >> 24) & 0xF) * 10 + ((last4[0] >> 16) & 0xF)
																													 : ((last4[0] >> 24) == 0x54 /*TDZx*/) ? c & 0xF : 4;
				...
				do {
					...
					if (
					sampleLength > 0
					&& (window[(p + 2) & WINDOW_ACCESS_MASK] > 0xF /*sample finetune, 0..0xF*/ || window[(p + 3) & WINDOW_ACCESS_MASK] > 0x40 /*sample volume, 0..0x40*/))
						break;
					length += sampleLength;
					j++;
				} while (j < 31);
