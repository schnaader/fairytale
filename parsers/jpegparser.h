...
private:
	enum Markers { SOF0 = 0xC0, SOF1 = 0xC1, SOF2 = 0xC2, DHT = 0xC4, SOI = 0xD8, EOI = 0xD9, SOS = 0xDA, DQT = 0xDB };
	uint8_t buffer[GENERIC_BUFFER_SIZE];
	off_t position;

public:
	explicit JpegParser(void) { priority = PARSER_PRIORITY_JPEG; }
						...
						while (!done && (bytesRead = (int)block->data->blockRead(&buffer[0], GENERIC_BUFFER_SIZE))) {
							for (int j = 0; !done && (j < bytesRead); j++) {
								...
								if (!isMarker)
									...
									(buffer[j] && ((buffer[j] & 0xF8) != 0xD0 /*skip Restart Markers RST0 to RST7*/) && ((progressive) ? (buffer[j] != DHT) && (buffer[j] != SOS) : true));
									found = (buffer[j] == EOI);
									isMarker = false;
								}
							}
