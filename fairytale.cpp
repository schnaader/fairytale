      ...
      default: {
        switch (block->type) {
          case BlockType::IMAGE: {
            ...
            switch (info->bpp) {
              case 32: { stats.img32++; stats.totals.img32+=block->length; break; }
              case 24: { stats.img24++; stats.totals.img24+=block->length; break; }
              case  8: {
                if (info->grayscale)
              ...
              case  4: { stats.img4++; stats.totals.img4+=block->length; break; }
              case  1: { stats.img1++; stats.totals.img1+=block->length; break; }
            }
            break;
          }
          case BlockType::AUDIO: {
            ...
            stats.mod++; stats.totals.mod+=block->length;
            break;
          }
          case BlockType::DDS: {
            stats.dds++; stats.totals.dds+=block->length;
            break;
          }
          case BlockType::JPEG: {
            stats.jpeg++; stats.totals.jpeg+=block->length;
            break;
          }
          default: {}
        }
